import unittest
from bizarro_lib.template.generic_template import GenericTemplate
...
class TestGenericTemplate(unittest.TestCase):
    ...
    def test_generic_template_one(self):
        default_action = self.button.create_url_button(
            "Boshe achi eka", self.url, is_default_action=True)
        payload = list()
        ...
        self.assertNotEqual(generic_payload, None,
    ...
    def test_generic_template_two(self):
        default_action = self.button.create_url_button(
            title="", url=self.url, is_default_action=True)
        buttons = list()
        for _ in range(0, 3):
    ...
    def test_generic_template_seven(self):
        default_action = self.button.create_url_button(
            "", self.url, is_default_action=True)
        buttons = list()
        for _ in range(0, 3):
        ...
        generic_payload = self.generic_template.generic_template(
            payload, shareable=True, image_aspect_ratio="square")
        self.assertNotEqual(generic_payload, None,
                            "Generic Payload is None! Crap!")
        status_code = self.facebook.send_message(generic_payload)
        self.assertEqual(status_code, 200,
    ...
    def test_generic_template_eight(self):
        ...
        generic_payload = self.generic_template.generic_template(
            payload, shareable=True, image_aspect_ratio="vertical")
        self.assertEqual(generic_payload, None,
                         "Generic Payload is None! Crap!")
        status_code = self.facebook.send_message(generic_payload)
        self.assertEqual(status_code, 400,
    ...
    def test_generic_template_ten(self):
        default_action = self.button.create_url_button(
            "", self.url, is_default_action=True)
        buttons = list()
        for _ in range(0, 3):
        ...
        payload = list()
        payload_title = "test_generic_template_ten "
        payload.append(self.generic_template.create_single_generic_elements(
            payload_title, buttons=buttons, default_action=default_action))
        generic_payload = self.generic_template.generic_template(
            payload, shareable=True)
        self.assertNotEqual(generic_payload, None,
                            "Generic Payload is None! Crap!")
    ...
    def test_generic_template_eleven(self):
        payload = list()
        payload_title = "test_generic_template_eleven "
        payload_subtitle = "Gello Hello - 11"
        payload.append(self.generic_template.create_single_generic_elements(
            payload_title, subtitle=payload_subtitle, image_url=self.url))
        generic_payload = self.generic_template.generic_template(
            payload, shareable=True)
        self.assertNotEqual(generic_payload, None,
                            "Generic Payload is None! Crap!")
